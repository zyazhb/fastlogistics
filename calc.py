...
for i in range(markets.shape[0]):
    for j in range(markets.shape[0]):
        D[i,j] = (markets[i,0]-markets[j,0])**2+(markets[i,1]-markets[j,1])**2
...
for iter in range(MaxGen):
    ...
    en = encode(pop, vk, mk)
    de = decode(en)
    fit_value = Objv(de, c1, c2, cv, cw, D, Ve, Me, av, aw, vk, mk, v, time_limit, time_cost)
...
def main():
    ...
    for i in range(len(Best_path)):
        print(f"第{Best_path[i][1]}种类型的车,路径为0->{map(Best_path[i][2:-1])}->0,运输的商品为{Best_path[i][2:-1]}")
        res.append(map(Best_path[i][2:-1]))
    print(f"最低成本为{Min_fit[-1]}")
    return res

    #--------------------------------画图------------------------------#
    colors = ['lightcoral','black','gray','lime','pink','gold','red','cadetblue','navy','brown','deeppink','green']
    linestyles = ['-','--','-.',':','-','--','-.',':','-','--','-.',':','-','--','-.',':']
    ...
    for i in range(len(Best_path)):
        ...
        for j in range(len(Best_path_che)-1):
            plt.plot([markets[Best_path_che[j],0],markets[Best_path_che[j+1],0]], [markets[Best_path_che[j],1],markets[Best_path_che[j+1],1]],color='k',linestyle='-.',linewidth='1')
        plt.plot([markets[0,0],markets[Best_path_che[0],0]],[markets[0,1],markets[Best_path_che[0],1]],color='k',linestyle='-.',linewidth='1')
        plt.plot([markets[0,0],markets[Best_path_che[-1],0]],[markets[0,1],markets[Best_path_che[-1],1]],color='k',linestyle='-.',linewidth='1')
